...
## Отменить ошибочные действия (например, последний коммит):
  🆘 Отмена действий и исправление ошибок
* git commit --amend --no-edit — добавить забытые файлы в последний коммит или просто изменить его состав без правки сообщения.
* git commit --amend -m "Новое сообщение" — быстро переименовать последний коммит.
* git restore <file> — вернуть файл к состоянию на момент последнего коммита (отменить незакоммиченные правки).
* git reset --soft HEAD~1 — «откатить» последний коммит. Сам коммит исчезнет, но все ваши изменения останутся в файлах (удобно, если нужно переделать работу и закоммитить иначе).
* git reset --hard HEAD~1 — полное удаление последнего коммита вместе со всеми изменениями.
  > **Важно**: Осторожно! Данные будет невозможно вернуть!
* git reset --hard origin/main — радикальный метод: привести локальное состояние ветки в точное соответствие с тем, что сейчас на GitHub (все локальные непушнутые правки удалятся).
...
### Gemini:
* git pull — «стянуть» свежие коммиты из облака.
  Как работает: Это связка команд git fetch (скачать) + git merge (слить).
  > **Важно**: Запускайте её, находясь в той ветке, которую хотите обновить (например, в main).
* git pull origin <имя_ветки> — забрать изменения из конкретной ветки удаленного репозитория.
  > Нюанс: Если вы правили код в локальной ветке и в это же время кто-то (или вы через интерфейс GitHub) изменил те же строки в облаке, git pull выдаст конфликт слияния. Git не поймет, чья версия главнее, и попросит вас выбрать вручную.

### Сократить классический вариант Яндекс Практикума.
В современных версиях Git, если вы находитесь в ветке my-branch, вы можете обновить её из main одной командой, не переключаясь туда-сюда:
* git fetch origin main — скачивает изменения main с сервера, но не переключает вас на неё.
* git merge origin/main — вливает скачанные изменения сразу в вашу текущую ветку.

## Разрешние конфликтов

* CONFLICT (content): Merge conflict in [файл]

1. Найти проблемные файлы
   * Команда git status покажет список файлов с пометкой both modified.
2. Открыть файл и выбрать версию
   * Внутри файла вы увидите специальные маркеры Git:
text
<<<<<<< HEAD
Ваш код (то, что было в текущей ветке)
=======
Чужой код (то, что вы пытаетесь влить)
>>>>>>> main
Используйте код с осторожностью.

   * Вам нужно вручную удалить эти маркеры и оставить только тот код, который должен выжить (или скомбинировать оба варианта).
3. Завершить слияние
   * После того как вы сохранили файл без маркеров:
   * git add <имя_файла> — так вы говорите Git: «Я всё исправил!».
   * git commit -m "Разрешить конфликт слияния" — создается финальный коммит слияния.
   💡 Лайфхаки для шпаргалки:
   * VS Code / WebStorm: Эти редакторы подсвечивают конфликты цветом. Там есть удобные кнопки: «Accept Current Change» (оставить своё) или «Accept Incoming Change» (взять чужое). Это гораздо быстрее, чем удалять маркеры вручную.

### 🚨 Экстренные команды при конфликтах
* git merge --abort — «Тревожная кнопка». Полностью отменяет попытку слияния и возвращает репозиторий в то состояние, которое было до ввода команды git merge. Полезно, если конфликтов слишком много и вы не готовы решать их прямо сейчас.
* git status — лучший друг в любой непонятной ситуации. Всегда подскажет, какие файлы конфликтуют и что нужно сделать (add или commit), чтобы завершить процесс.
* git diff — покажет список всех изменений, которые вызвали спор между ветками, чтобы вам было проще принять решение.
