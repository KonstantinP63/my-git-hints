...
## Правило исполнения команд Git
Большинство команд (status, remote, commit, push) выполняются только **внутри** папки проекта. 
*   **Исключение**: `git clone` (выполняется в родительской папке).
*   **Проверка**: Перед командой убедитесь, что терминал показывает имя текущей ветки в скобках.
